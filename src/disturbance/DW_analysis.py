...
Multiple sites (async export to Google Drive):
...
def get_single_site_lc_change(year, site_id, sites,
    ...
    """
    yr = int(year)
    site = sites.filter(ee.Filter.eq("SITE_ID", site_id))
...
def export_lc_change_multiple_sites(year, sites, description=None, folder='EarthEngine',
                                anom_lower=30, conf_lower=400,
    ...
    stats = combined.reduceRegions(
        collection=sites.select(['NAME', 'SITE_ID']),
        reducer=ee.Reducer.frequencyHistogram(),
        scale=30,
        tileScale=4,
    )
...
def load_lc_change_site_export(csv_path, year, min_disturbed_pixels=10):
    """
...
def export_lc_change_grid(year, grid, description=None, folder='EarthEngine',
                                anom_lower=30, conf_lower=400,
                                min_post_months=2, post_window=4):
    """
    Submit an async GEE Export task for pixel grid LC change analysis.
...
def load_lc_change_grid_export(csv_path, year, min_disturbed_pixels=10):
    """
    Parse a CSV exported by export_lc_change_grid into the standard result dict.
